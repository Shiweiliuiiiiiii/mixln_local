...
def evaluate_ppl(max_length=256, batch_size=128, device="cuda:0", del_index=-1):
    # Load model
    model = LlamaForCausalLM.from_pretrained("/scratch/shiwei/shiwei/llama1b/1b_res_pre_lr5e-4/model_50001/", torch_dtype=torch.float16)
    model = model.to(device)
    model.eval()
    
    model_back = LlamaForCausalLM.from_pretrained("/scratch/shiwei/shiwei/llama1b/1b_res_pre_lr5e-4/model_50001/", torch_dtype=torch.float16)
